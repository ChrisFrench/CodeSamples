...
class Assets extends \Dsc\Mongo\Collections\Assets 
{
    ...
    public static function createFromUpload( array $file_upload, $options=array() )
    {
        ...
        $name =  !empty($options['name']) ? $options['name'] : $file_upload['name'];
        // Do the upload        
        $model = new static;
        $grid = $model->getDb()->getGridFS( $model->collectionNameGridFS() );
        $file_path = $model->inputFilter()->clean($file_upload['tmp_name']);
        $name = $model->inputFilter()->clean($name);
		$buffer = file_get_contents($file_upload['tmp_name']);
		
		
		$picture = \PHPImageWorkshop\ImageWorkshop::initFromString($buffer);
		
		$v2 = \PHPImageWorkshop\ImageWorkshop::initFromPath('/var/www/ryst.cc/public/wm/v2.png');
		$v2->opacity(50);
		$das = \PHPImageWorkshop\ImageWorkshop::initFromPath('/var/www/ryst.cc/public/wm/das.png');
		$das->opacity(50);
		
		// We add the text layer 12px from the Left and 12px from the Bottom ("LB") of the norway layer:
		$picture->addLayerOnTop($v2, 12, 12, "LB");
		$picture->addLayerOnTop($das, 12, 12, "RB");
		
		$watermarked = $picture->getResult();
		
		$image = new \Dsc\Image($watermarked);
