...
{   
     ...
     public function dispatch($tag) {
     	...
     	if(!empty($user->id)) {
     		...
     		$user = (new \Rystband\Models\Users)->setState('filter.id', $this->auth->getIdentity()->id)->getItem();
     		 
     		//if(!empty($user->id)) {
     			//We are logged in
     			$this->app->set('user',$user);
     			if((string) $user->id  == (string) $tag->{'attendee.id'} ) {
     ...
     protected function experience($user,$tag) {
     
     	
     	$view = \Dsc\System::instance()->get( 'theme' );
     	 
     	echo $view->setVariant('content')->render($user->{'experience.view'});
     }
     ...
     protected function login($tag) {

          $view = \Dsc\System::instance()->get( 'theme' );
          echo $view->render('Rystband/Site/Views::event/attendee/signin.php');
     }
