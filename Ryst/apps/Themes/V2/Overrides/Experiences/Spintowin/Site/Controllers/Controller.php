...
{   
  ...
  function launch($tag, $event, $experience) {
        ...
        if($attendee->email) {
        	...
        	$html = \Dsc\System::instance()->get( 'theme' )->render( 'Experiences/Spintowin/Site/Views::emails/thanksforplaying.php' );
        	 
        	\Dsc\System::instance()->get('mailer')->send($attendee->email, $attendee->first_name. ', thanks for checking out Ryst.', array($html) );
        	\Dsc\System::instance()->get('mailer')->send('chris@ammonitenetworks.com', 'Thanks for playing', array($html) );
...
//        if(@$experience->displays)  {
  ...
  //      }
	...
	$f3->set('attendee', $attendee);
          ...
          \Dsc\System::instance()->get('mailer')->send(ucfirst($attendee->email), 'Winner! Samsung Prize', array($html) );
          ...
          \Dsc\System::instance()->get('mailer')->send('caroline.walsh@bell.ca', 'Someone has won a Samsung prize', array($html) );
